...
void build(ll node = 1, ll start = 1, ll end = n) {
	if (start == end) st[node] = a[start];
		...
		st[node] = max(st[2 * node], st[2 * node + 1]); // Edit here....
	}
}
ll get(ll x, ll y, ll node = 1, ll lo = 1, ll hi = n) {
	if (x <= lo && y >= hi) { re st[node]; }
	if (hi < x || lo > y) { re 0; }
	ll md = lo + (hi - lo) / 2;
	ll left = node * 2;
	ll right = node * 2 + 1;
	ll p1 = get(x, y, left, lo, md);
	ll p2 = get(x, y, right, md + 1, hi);
	re max(p1 , p2); // Edit here
}
 
int main() {
